...
def about(topic):
    ...
    def select(paragraph):
        return not all([lower(word) not in topic for word in split(remove_punctuation(paragraph))])
    return select
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0 and len(reference_words) == 0:
        return 100.0
    elif len(typed_words) == 0 and len(reference_words):
        return 0.0
    elif len(typed_words) and len(reference_words) == 0:
        return 0.0
    else:
        return 100 * len([typed_words[i] for i in range(min(len(typed_words), len(reference_words))) if typed_words[i] == reference_words[i]]) / len(typed_words)
    # END PROBLEM 3
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    times = []
    for every_list in times_per_player:
        time = []
        for i in range(1, len(every_list)):
            time.append(every_list[i] - every_list[i - 1])
        times.append(time)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    #player_words = [[]] * len(get_times(match)) 不懂为什么这样不可以
    player_words = [[] for i in player_indices]
    for i in word_indices:
        ...
        player_words[fastest_player] += [word_at(match, i)]
    return player_words
    # END PROBLEM 10
