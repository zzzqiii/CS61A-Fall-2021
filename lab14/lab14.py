...
def prune_min(t):
    ...
    """
    if t.branches:
        t.branches = [min([b for b in t.branches], key = lambda t: t.label)]
...
def add_trees(t1, t2):
    ...
    4
      ...
      5
    """
    new = Tree(t1.label + t2.label)
    def helper(t1, t2):
        for items in zip(t1.branches, t2.branches):
            tree = add_trees(items[0], items[1])
            new.branches.append(tree)
        if len(t1.branches) != len(t2.branches):
            l = min(len(t1.branches), len(t2.branches))
            m = max([t1, t2], key = lambda t: len(t.branches))
            new.branches += m.branches[l:]
...
def match_url(text):
    ...
    path = r'/(?:\w+/)*(?:\w+\.html)?'
    anchor = r'#[\w\d\-_]+'
    #return re.findall(rf"^(?:{scheme})?{domain}(?:{path})?(?:{anchor})?$", text)
