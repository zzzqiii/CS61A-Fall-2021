...
def store_digits(n):
    ...
    if n < 10:
        ...
        while tmp:
            length += 1
            tmp //= 10
        return Link(n // (10 ** (length-1)), store_digits(n % (10 ** (length-1))))
...
def bst_min(t):
    if (t.is_leaf()):
        ...
        return min(t.label, min([bst_min(t) for t in t.branches]))
def bst_max(t):
    if (t.is_leaf()):
        return t.label
    else:
        return max(t.label, max([bst_max(t) for t in t.branches]))
