...
def match_time(text):
    ...
    ['2:00']
    """
    #注意要用非捕获组，否则会得到捕获组里的内容
    return re.findall(r'(?:[01]?[0-9]|2[0-4]):[0-5][0-9](?:AM|PM)?', text)
...
def most_common_code(text):
    ...
    """
    freq = {}
    all_code = area_codes(text)
    for code in all_code:
        freq[code] = all_code.count(code)
    return max(freq.keys(), key = lambda key: freq[key])
    #improved solution
    #return max(all_code, key = lambda code: all_code.count(code))